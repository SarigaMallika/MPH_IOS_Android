...
Scenario: Verify the scenario when entering different data in 'Password' and 'Confirm Password'
 ...
 Scenario: Validate the 'Apply' button disabled when Referral code field is blank
    When Referral code is blank
    Then Verify that the Apply button is disabled
    
  Scenario: Validate that the user should get a validation message on entering invalid referral code
